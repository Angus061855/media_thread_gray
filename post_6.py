...
NOTION_POST_DB_ID  = os.environ["NOTION_DATABASE_ID_6"]
...
def post_to_threads(content):
    ...
    content = normalize_content_format(
        content
    )
    ...
    for i, text in enumerate(posts):
        ...
        for attempt in range(3):
            ...
            pub_res = requests.post(
                (
                    f"https://graph.threads.net/"
                    f"v1.0/{THREADS_USER_ID}/"
                    f"threads_publish"
                ),
                data={
...
if __name__ == "__main__":
    ...
    print(
        f"🎲 隨機選中"
        f"（共 {len(valid_posts)} 筆有內容）"
    )

    print(
        "讀到的內容預覽："
        f"{repr(target_content[:200])}"
    )
    ...
    try:
        ...
        send_telegram(
            "✅ Thread gray 6 給文章 發文成功！"
        )

    except Exception as e:

        error_msg = (
            "❌ Thread gray 6 給文章 發文失敗！"
            f"\n錯誤原因：{str(e)}"
        )
